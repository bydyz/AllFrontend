...
const store = createStore({
  ...
  getters: {
    ...
    message(state, getters, rootState, rootGetters) {
      ...
      console.log("通过rootState", rootState.counter.count)

      console.log("通过rootGetters", rootGetters["counter/doubleCount"])

      return `name:${state.name} level:${state.level} friendTotalAge:${getters.totalAge}`
    },
    // 3.getters是可以返回一个函数的, 调用这个函数可以传入参数(了解)   counterStore.getFriendById(111)
    getFriendById(state) {
      return function(id) {
