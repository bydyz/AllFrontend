<template>
  <BaseTreeEdit
      :value="value"
      @input="$emit('input')"
      @update:deleteList="$emit('update:deleteList', $event)"
      :allowDrop="allowDrop"
      :max="max"
      :border="border">
    ...
    <template slot="row" slot-scope="{data}">
      <div class="col-name">
        <el-input v-if="data.edited" v-model="data.navbarName" placeholder="导航名称" size="small" ref="input"
      ...
      <div class="col-link">
        <template v-if="data.edited">
          ...
          <el-select v-model="data.linkUrl" size="small" class="width-120" @change="onSave(data)">
            <el-option v-for="(item,index) in navbarSelectList" :key="index" :label="item.navbarName"
                       :value="item.linkUrl"></el-option>
          </el-select>

          <!--2.2 自定义链接输入-->
          <el-input v-if="data.linkUrl === customUrl"
                    v-model="data.customLinkUrl" size="small" @blur="onSave(data)"
                    placeholder="请输入自定义链接" class="width-240" maxlength="100" show-word-limit></el-input>

          <!--2.3 微页面-->
          <el-select v-else-if="data.linkUrl === wPageUrl"
                     v-model="data.linkId" size="small" class="width-240">
...
export default {
  ...
  computed: {
    ...mapGetters({
      navbarSelectList: 'common/navbarOrgSelectList',
      wPages: 'common/waOrgPages'
    })
  },
  methods: {
    async onSave(data) {
