...
const mount = (vnode, container) => {
  ...
  const el = vnode.el = document.createElement(vnode.tag);      //对vnode.el进行赋值，让vnode中也保存一个真实dom
  ...
  if (vnode.props) {
    for (const key in vnode.props) {
      ...
      if (key.startsWith("on")) { // 对事件监听的判断
        ...
        el.setAttribute(key, value);    //给el元素设置属性
      }
    }
  }
...
const patch = (n1, n2) => {
  // 新vdom n2和原来的vdom n1类型（tag）不同，直接用n2提换原来的n1去挂载
  if (n1.tag !== n2.tag) {
    const n1ElParent = n1.el.parentElement;
    n1ElParent.removeChild(n1.el);
    mount(n2, n1ElParent);
  } else {
    // 1.取出n1的element对象, 并且在n2和el中进行保存（n2没有进过mount函数，故没有el）    此处是对象的引用，改动el时，n2和n1的el均会改变
    ...
    // 3.处理children   children一般就是字符串 或者 数组，需要用插槽时才会是对象
