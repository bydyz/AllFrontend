export async function fetchSchemaStream(
  ...
  const response = await fetch(url, {
    method: 'POST',
    headers: { 'Authorization': 'Bearer sk-api-_xN0p-n2TWAMKbiyNl9CEo3YIW5B0HA_vkp-LenMAHRxCHTo67IKGS5ZM_j8ZUb6ywjjkStF5yCnI4afjGpCWUB8koYSlp55qbKFtscnBnoRwbLPvyea8EM', 'Content-Type': 'application/json' },
    body: JSON.stringify({
      messages: [{ role: 'user', content: userInput }],
      model: 'MiniMax-M2.5',
      stream: true, // 启用流式响应，以便实时获取 schema 数据
    }),
  });
  ...
  const reader = response.body.getReader(); // 获取响应体的读取器，用于流式读取数据块
  const decoder = new TextDecoder('utf-8'); // 创建文本解码器，将二进制数据流解码为 UTF-8 字符串
  let buffer = '';
