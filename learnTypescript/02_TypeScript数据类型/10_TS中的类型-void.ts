...
// 在 TypeScript 中，如果你对一个函数的类型限制是 type FnType = () => void，这表示该函数的返回类型是 void，即没有明确的返回值。在这种情况下，函数可以有返回语句，但返回的值会被忽略。
type FnType = () => void
 
const foo0: FnType = () => {
  return 123
}
//! 上面的例子中，尽管函数体中有 return 123; 这个返回语句，但由于函数的类型限制是 () => void，TypeScript 会忽略返回值。如果你尝试使用该函数的返回值，TypeScript 会给出相应的类型错误。
